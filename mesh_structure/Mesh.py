...
class Mesh:
    ...
    def get_unique_hash_key_count(self):
        return len(self.all_countour_nodes)
    ...
    def create_vertex(self, x, y):
        ...
        if v in self.sorted_vertex_lists:
            return self.sorted_vertex_lists[v]
        else:
            self.sorted_vertex_lists.add_to_x_sorted(v)
            self.sorted_vertex_lists.add_to_y_sorted(v)
        return v
    ...
    def create_mesh_contour(self):
        ...
        self.contour = MeshContour.MeshContour(contour_vertices)

    def __get_top_border(self):
        border_vertices = np.empty(dtype=object, shape=0)
        current_edge = self.__get_upper_left_vertex().get_longest_edge_in_direction(Direction.right)
        ...
        while current_edge.v1.x < upper_right_vertex.x:
            if current_edge.v2.x < upper_right_vertex.x:
                try:
                    current_edge = current_edge.v2.get_longest_edge_in_direction(Direction.right)
    ...
    def __get_right_border(self):
        border_vertices = np.empty(dtype=object, shape=0)
        current_edge = self.__get_upper_right_vertex().get_longest_edge_in_direction(Direction.bottom)
        ...
        while current_edge.v2.y > lower_right_vertex.y:
            if current_edge.v1.y > lower_right_vertex.y:
                try:
                    current_edge = current_edge.v1.get_longest_edge_in_direction(Direction.bottom)
                    border_vertices = np.append(border_vertices, current_edge.v2)
                except ValueError:
                    continue
            else:
    ...
    def __get_bottom_border(self):
        border_vertices = np.empty(dtype=object, shape=0)
        current_edge = self.__get_lower_right_vertex().get_longest_edge_in_direction(Direction.left)
        border_vertices = np.append(border_vertices, current_edge.v2)
        ...
        while current_edge.v2.x > lower_left_vertex.x:
            if current_edge.v1.x > lower_left_vertex.x:
                try:
                    current_edge = current_edge.v1.get_longest_edge_in_direction(Direction.left)
                    border_vertices = np.append(border_vertices,
                                                current_edge.v2)
                except ValueError:
                    continue
    ...
    def __get_left_border(self):
        border_vertices = np.empty(dtype=object, shape=0)
        current_edge = self.__get_lower_left_vertex().get_longest_edge_in_direction(Direction.top)
        border_vertices = np.append(border_vertices, current_edge.v1)
        ...
        while current_edge.v1.y < upper_left_vertex.y:
            if current_edge.v2.y < upper_left_vertex.y:
                try:
                    current_edge = current_edge.v2.get_longest_edge_in_direction(Direction.top)
                    border_vertices = np.append(border_vertices,
                                                current_edge.v1)
                except ValueError:
                    continue
