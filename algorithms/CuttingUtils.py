...
class SlicePathsFinder:
            ...
            for index_end_v in range(index_start_v + 1, len(self.contour)):
                ...
                if len(possible_directions) > 0:
                    for direction in possible_directions:
                        travel_vertex = self.__travel_to_next_vertex(start_v, direction)
                        used_cross_vertices = [start_v]
                        self.__find_path(travel_vertex, direction.get_oposite_direction(), used_cross_vertices)
        return self.possible_paths
    
    
    def __find_path(self, v, arrival_direction, used_cross_vertices):
        if v in self.end_vertices_list:
            used_cross_vertices = self.__copy_used_vertices_list_and_append(v, used_cross_vertices)
        ...
        elif self.__is_there_any_way(v, arrival_direction):
            possible_travel_directions = self.__get_possible_travel_directions(v, arrival_direction)     
            used_cross_vertices = self.__copy_used_vertices_list_and_append(v, used_cross_vertices)
            is_curr_v_useful = False
            for direction in possible_travel_directions:
                travel_vertex = self.__travel_to_next_vertex(v, direction)
                if self.__find_path(travel_vertex, direction.get_oposite_direction(), used_cross_vertices):
    ...
    def __copy_used_vertices_list_and_append(self, v, used_vertices):
        used_vertices_copy = copy(used_vertices)
        used_vertices_copy.append(v)
        return used_vertices_copy
    
    
    def __travel_to_next_vertex(self, v, direction):
    ...
    def __get_possible_travel_directions(self, v, arrival_direction):
    ...
    def __is_there_any_way(self, v, arrival_direction):
...
class ContourSlice:
    ...
    def slice_contour(self):
        contour1, contour2 = self.__slice_and_create_new_contours()
    ...
    def __slice_and_create_new_contours(self):
        start_v = self.path[0]
        end_v = self.path[len(self.path) - 1]
        index_start_v = self.contour.get_index_of(start_v)
        index_end_v = self.contour.get_index_of(end_v)
        countour_part_1 = self.contour.get_vertices_beetween(index_start_v + 1, index_end_v)
        new_contour_1 = np.append(countour_part_1, self.path[::-1])
        countour_part_2 = self.contour.get_vertices_beetween(0, index_start_v) 
